...
明细支持点击表头本地排序。双击未删除明细时，以该行 `ApplyFormNO` 和 `Apply_Time` 打开或激活“输血结果查询”，清空病人编号和姓名筛选，将申请状态设为“全部”，按申请日期当天精确查询并选中对应申请单，同时刷新右侧详情；不自动切换右侧页签。已删除明细不跳转，并提示“输血结果查询”当前不显示已删除记录。CSV 导出使用当前备血类型过滤和当前排序后的内存明细，不再次查询数据库。

模块间跳转通过 `blood_module.h` 中的 `WM_BLOOD_OPEN_REQUEST` 和 `BloodRequestOpenTarget` 完成。备血统计创建目标对象并传递申请单号、申请时间；输血结果查询接收消息后负责释放目标对象、执行精确查询和定位行。该接口不直接访问另一模块的窗口状态，也不改变输血结果查询当前页签。
...
| 双击未删除明细 | 打开或激活“输血结果查询”，按申请单号和申请日期定位并选中对应行，右侧页签保持原状态 |
| 双击已删除明细 | 提示当前输血结果查询不显示已删除记录，不执行跳转 |
| 结束日期当天 23:59:59 | 统计 |
| 结束日期次日 00:00:00 | 不统计 |
