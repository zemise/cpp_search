...
- 统计目标：按同一病人号识别 24 小时内累计申请血液制品折算量达到或超过 `1600 ml` 的患者申请事件。
...
- 24 小时事件划分、日期归属、申请状态范围、固定阈值、院区筛选和页面明细能力均已确认，第一版核心业务口径已具备开发条件。

## 已确认业务口径

截至 2026-08-07，以下口径已确认，可直接作为第一版开发依据：
...
| 阈值 | 固定为 `1600 ml`，折算总量 `>= 1600 ml` 均命中 |
| 实际单位值域 | `ML / U / 治疗量` |
| `ML` 换算 | `ApplyNum × 1 ml` |
...
例如 `4 U + 2 治疗量 + 300 ML = 4×200 + 2×250 + 300 = 1600 ml`，达到固定阈值，应进入大量输血统计。
...
- 正式命中列表展示累计折算量大于或等于 `1600 ml` 的事件。
...
- 大于或等于 `1600 ml` 均命中。
- 等于 `1600 ml` 必须命中。
- 第一版阈值固定，不提供可修改的阈值输入框。
- 汇总、明细和导出必须显示本次查询使用的阈值，防止不同口径结果混用。
...
1. 将已确认规则定义在 C++ 聚合逻辑中，规则版本记为 `v2`。
2. 页面和 CSV 显示或记录规则版本，保证历史结果可追溯。
...
筛选区下方状态行显示查询进度、院区、成分开关状态、固定阈值和折算规则版本。
...
| 指标 | 口径 |
| --- | --- |
| 大量输血事件数 | 完整折算且达到或超过 `1600 ml` 的事件数 |
| 涉及患者数 | 命中事件中的唯一病人号数 |
...
7. 阈值固定为 `1600 ml`，折算量 `>= 1600 ml` 命中。
...
- 单位规范化后按固定规则折算：`ML×1 / 普通U×200 / 冷沉淀U×20 / 治疗量×250`，规则版本为 `v2`。
- 无法折算的数据明确进入异常列表。
