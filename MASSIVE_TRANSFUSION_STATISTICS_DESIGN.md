...
- 当前实现：页面可选择“申请量对照/实际输血量”。实际输血口径以已审核交叉配血记录为事实、按唯一血袋计量，支持四种事件时间、异常核查、事件/血袋两级 Excel 工作簿；申请量逻辑原样保留用于对照。
...
- 数据来源：
  ...
  - 血袋表 `LS_XK_BloodInfo`、出库表 `LS_XK_BloodOutInfo`、血液成分字典 `LS_XK_B_CompositionInfo` 和成分类型字典 `LS_XK_B_CompositionType`。
- 可复用模块：
...
- 单位值域和换算规则已确认：当前实际单位只有 `ML / U / 治疗量`；普通制品按 `1 ML = 1 ml`、`1 U = 200 ml`、`1 治疗量 = 250 ml` 折算，成分类型 ID 为 `3` 的冷沉淀按 `1 U = 20 ml` 折算。制品名称仅用于展示，不参与分类、兜底或异常识别。
...
| 容量与类型来源 | `BloodInfo.CompositionID -> CompositionInfo.Norm + CompositionInfo.Unit + CompositionInfo.CompositionTypeID` |
...
| 血小板和冷沉淀 | `CompositionTypeID=3/4` 分别识别冷沉淀/血小板；默认不计量，勾选后计量；冷沉淀 `1U=20ml` |
...
| `LS_XK_B_CompositionInfo` | 提供 `Blood_Composition / Norm / Unit / CompositionTypeID`；名称用于展示，规格、单位和类型 ID 用于实际血袋计量 |
| `LS_XK_B_CompositionType` | 成分类型字典；现场只读核查确认 `1=红细胞、2=血浆、3=冷沉淀、4=血小板` |
...
实际输血事件的“制品构成”只由 `CompositionInfo.Blood_Composition` 及该血袋的 `Norm + Unit` 折算量汇总，不读取申请子表成分。页面和事件 Excel 工作簿在实际口径下显示“实际输血制品构成”，切换到申请量对照时显示“申请制品构成”，避免两种口径在展示层混淆。
...
一次查询只能使用一种时间口径。事件 ID、排序、日期范围、24小时窗口、状态提示和 Excel 工作簿都必须记录本次选择，不能在同一次查询中混合多种时间来源。
...
LS_XK_BloodCrossMatch.BloodInID
    ...
    -> LS_XK_B_CompositionType.ID
```

`Norm` 作为每袋数量，`Unit` 作为原单位，`CompositionTypeID` 作为唯一制品分类依据，`Blood_Composition` 只用于页面和导出展示。缺少或无法识别类型 ID、`Norm`、`Unit` 时不按名称兜底，也不按 `0` 静默处理；该项不计量，事件标记为“实际量不完整”。名称为空不影响分类或折算，展示时使用“未命名制品”。

折算规则暂沿用现有版本：

| 实际血袋成分与单位 | 折算 |
| --- | ---: |
| 已识别类型，单位为 `ML` | 数量 `× 1 ml` |
| `CompositionTypeID=3`，单位为 `U` | 数量 `× 20 ml` |
| 其他已识别类型，单位为 `U` | 数量 `× 200 ml` |
| 已识别类型，单位为 `治疗量` | 数量 `× 250 ml` |

“血小板和冷沉淀”开关继续保留，以 `CompositionTypeID=3` 识别冷沉淀、`CompositionTypeID=4` 识别血小板。未勾选时相关血袋保留明细但不贡献事件实际总量。`CompositionTypeID=2` 的血浆正常计量，即使展示名称中包含“冷沉淀”。
...
- 支持将当前明细导出为 Excel 工作簿，并可跳转到输血结果查询核查原始申请单。
...
| `CrossNum` | 配血量，本模块不使用 |
| `CompositionBig_ID` | 成分大类 ID，申请量对照的唯一制品分类依据；`1=红细胞、2=血浆、3=冷沉淀、4=血小板` |
...
| `CompositionTypeID` | 成分类型 ID，实际输血量口径的唯一制品分类依据 |
| `Del_Flat` | 删除标志 |

### LS_XK_B_CompositionType

现场只读核查确认该表是成分类型字典。本项目仅记录统计所需的非隐私业务编码，不记录创建人、删除人等现场维护信息：

| `ID` | `CompositionType` | 统计含义 |
| ---: | --- | --- |
| `1` | 红细胞 | 普通计量制品 |
| `2` | 血浆 | 普通计量制品；包括名称中含“冷沉淀”的去冷沉淀血浆 |
| `3` | 冷沉淀 | 受页面开关控制；`U×20ml` |
| `4` | 血小板 | 受页面开关控制 |

数据库未为 `LS_XK_B_CompositionInfo.CompositionTypeID` 建立物理外键，本模块按上述已确认的逻辑关系读取。制品名称不参与类型兜底或异常识别；类型 ID 缺失或超出已确认值域时，该项不计量并按总量不完整处理。
...
姓名不参与患者归并，也不改变事件窗口、输血量、阈值命中或数据完整性。事件建立后，程序按事件时间顺序遍历申请或实际血袋，收集每条记录关联申请单的非空姓名，按首次出现顺序去重：

- “姓名”列显示事件时间顺序中最后出现的非空姓名；只有一个姓名时即为该姓名。
- “姓名数”作为默认可见的独立窄列，显示事件内唯一非空姓名数；大于 `1` 即提示存在姓名变化。
- 完整姓名链保留在事件数据、Excel 和逐袋/逐项明细中；第二张列表上方以“最后姓名（其他姓名）”紧凑展示，只有一个姓名时省略括号。
- 空姓名不作为一个姓名变体；全部为空时显示姓名为空、姓名数为 `0`。
- 事件明细继续保留每张申请单或每袋血对应的原始姓名，不用事件显示姓名覆盖。
- 多姓名只是身份信息提示，不标记“总量不完整”，也不进入容量或类型异常核查。
- 不读取身份证或住院患者表修正姓名，不按姓名拆分事件，患者键仍为去空格后的 `Patient_NO`。
...
- 查询状态和两级 Excel 工作簿必须显示本次查询使用的完整条件，防止不同口径结果混用。
...
第一版不提供改变统计状态口径的筛选项，保证所有用户使用相同统计口径。事件内申请单明细和 Excel 工作簿必须包含原始状态，并为已驳回申请显示“不计量”。
...
2. 制品类型只读取 `CompositionBig_ID`；`CompositionBig_ID=3` 且单位为 `U` 时使用冷沉淀专用因子。
...
当前折算规则按“`CompositionBig_ID` + 规范化申请单位”匹配：
...
- `U` 保持为 `U`。
- `治疗量` 保持原值。
- `CompositionBig_ID=3` 的制品在单位为 `U` 时使用专用因子 `20`；其他已识别类型的 `U` 使用因子 `200`。
- `CompositionBig_ID=3/4` 分别代表冷沉淀和血小板，受页面“包括血小板和冷沉淀”开关控制。
- `ApplyComposition` 仅用于展示，不用于分类、旧数据兜底或异常名称匹配。
...
当前共享折算规则版本 `v4` 内置以下已经确认的换算：

| 申请成分识别 | 申请单位 | 折算 |
| --- | --- | --- |
| 已识别成分大类 | `ML` | `ApplyNum × 1` |
| `CompositionBig_ID=3` | `U` | `ApplyNum × 20` |
| 其他已识别成分大类 | `U` | `ApplyNum × 200` |
| 已识别成分大类 | `治疗量` | `ApplyNum × 250` |

页面默认不勾选“包括血小板和冷沉淀”。不勾选时，`CompositionBig_ID=3/4` 的成分不计入事件总量和计量制品项，但仍保留在成分明细与 Excel 工作簿中，并显示原始数量、单位、类型 ID、换算因子、折算量及“未勾选，不计量”状态；这些排除项本身不造成事件“总量不完整”。勾选后两类制品正常参与统计。类型 ID 缺失或未知时不使用名称兜底，该项不计量并使事件总量不完整。

### 配置存放方案

推荐优先级：

1. 将已确认规则定义在 C++ 聚合逻辑中，当前共享规则版本记为 `v4`。
2. Excel 工作簿记录规则版本，保证历史结果可追溯；页面状态行不重复展示。
...
筛选区下方状态行显示查询进度；完成后仅显示统计口径、事件时间、院区、事件/异常/核查数量、原始记录数和耗时。成分开关、统计条件直接以筛选控件为准，折算规则版本保留在 Excel 工作簿元数据中。
...
| 院区 | 根据首张申请的申请科室派生 |
| 病人号 | 患者键 |
| 姓名 | 事件时间顺序中最后出现的非空姓名；仅表示最近记录姓名，不认定为权威姓名 |
| 事件内全部姓名 | 非空姓名按首次出现顺序去重并以箭头连接 |
| 姓名数 | 事件内唯一非空姓名数；默认在姓名列旁显示 |
| 是否多姓名 | 姓名数大于 `1` 时为“是”；仅作提示 |
...
本轮界面优化只调整展示层，不改变查询 SQL、24 小时事件算法、统计阈值、折算规则、异常判定或 Excel 工作簿字段。
...
事件主列表：默认只显示状态、姓名、姓名数、病人号、事件起始时间、
            总量、袋数/项数、制品构成和提示。

[当前事件血袋/成分] [全部血袋/成分] [异常核查]
[ ] 完整视图    当前患者、病人号、折算量和袋数/项数
逐袋或逐申请成分核心明细
```
...
默认使用“简洁视图”，只展示 9 个直接用于识别和判断事件的字段：状态、姓名、姓名数、病人号、事件起始时间、总量、袋数/项数、制品构成和提示。其中“袋数/项数”与“实际输血制品构成/申请制品构成”随统计口径动态切换；“提示”将多姓名、跨科室、需核查和量不完整合并为简短标签，没有提示时显示 `-`。

院区、事件内全部姓名、是否多姓名、患者类型、窗口结束时间、最后计量时间、关联申请单数、核查记录数、首袋申请单、全部申请单、申请科室、床号、申请状态和数据状态作为低频核查列，简洁视图下隐藏。勾选明细页签右侧的“完整视图”后，两张 ListView 立即恢复全部字段及原始字段顺序；取消勾选则恢复简洁列和面向业务阅读的列顺序。切换只改变显示宽度与顺序，不重新查询、不修改内存数据，也不影响排序数据、导出或跨模块跳转。
...
页签右侧使用紧凑提示，格式为“最后姓名（其他姓名） | 病人号 | 折算量 | 袋数/项数”；其他姓名按首次出现顺序连接，没有其他姓名时省略括号。这里不再显示事件起止时间。选择不同事件时立即更新提示和当前事件明细；切换到异常核查时显示核查记录数量。

逐项明细也默认使用简洁视图：实际输血量口径展示事件时间、血液制品、规格、折算量、血袋号、申请单号、申请科室和状态；申请量对照展示申请时间、血液制品、规格、折算量、申请单号、申请科室和状态。规格由原始数量和单位紧凑拼接，例如 `4U`、`300ML`。事件起点、院区、患者信息、计量标志、交叉配血 ID、血袋 ID、产品码、各业务时间、换算因子等核查字段在简洁视图隐藏，勾选“完整视图”即可查看。

“完整视图”只服务于页面核查。事件和明细 Excel 始终导出全部既有字段，不依据当前勾选状态删减列，避免界面偏好改变留档口径。

### 展示层实现整理

- 院区、统计口径、比较方式和事件时间均由统一的“显示文字 + 查询值”选项表驱动，避免下拉框序号、查询参数和状态文字分别维护后发生偏差。
- 简洁视图只向 ListView 填充当前展示所需的核心单元格；完整事件、血袋及申请成分对象始终保留在内存中，Excel 直接读取内存对象。勾选“完整视图”时再补齐隐藏单元格，因此默认查询后的界面填充量更小。
- ListView 批量刷新统一使用暂停重绘的作用域保护，退出填充流程时必定恢复重绘；“全部血袋/全部成分”在合并事件明细前先预留容量，减少重复内存分配。
- 事件排序按字段类型执行：总量、姓名数、申请单数、袋数/项数和核查数使用数值比较，其余字段使用稳定文本比较。排序后按 `event_id` 恢复原选中事件，不再强制跳到第一行，并只在真正选中新行时刷新下方明细。
- 查询结果接收、导出按钮状态和状态文字集中处理。后台查询结果使用自动释放对象管理，窗口已关闭或不再属于原页面时不投递结果，降低异步结束阶段的资源泄漏风险。
- 事件与明细导出共用表头和查询元数据生成逻辑；实际输血量使用“血袋明细”，申请量对照使用“申请成分明细”，页面提示、工作表名称和默认文件名保持一致。

### 后续增强

- 在“完整视图”基础上按实际使用反馈评估是否需要逐列选择和列宽持久化。
...
导出格式：标准 OOXML `.xlsx` 工作簿。

默认文件名：

```text
开始日期-结束日期大量输血统计明细-{全部|老院|新院}.xlsx
```
...
| 空申请时间 | 无法建立窗口，进入异常列表 |
| 同一事件内出现多个非空姓名 | 仍按病人号聚合；列表显示最后出现的姓名及姓名数，选中事件后展示完整变化链，不影响完整性与异常数 |
...
| `2 U` | 折算为 `400 ml` |
| 成分类型 ID 为 `3` 的 `2 U` | 折算为 `40 ml` |
| `2 治疗量` | 折算为 `500 ml` |
| `800 ML + 2 U + 2 治疗量` | 折算为 `1700 ml`，命中 |
| 未勾选“包括血小板和冷沉淀” | 成分类型 ID 为 `3/4` 的行保留明细但不计量 |
...
- 申请量按 `CompositionBig_ID`、实际输血量按 `CompositionTypeID` 分类；ID 为 `3/4` 时由默认不勾选的“包括血小板和冷沉淀”开关控制，排除时保留明细且不影响数据完整性判断。
- 制品名称只用于展示，不参与分类、旧数据兜底或异常名称匹配；类型 ID 缺失或未知时不计量，并将事件标记为总量不完整。
- 单位规范化后按固定规则折算：`ML×1 / 普通U×200 / 类型3冷沉淀U×20 / 治疗量×250`，当前共享规则版本为 `v4`。
- 无法折算的数据明确进入异常列表。
...
- 模块只读 LIS，支持事件级和成分级 Excel 工作簿、事件内申请成分查看、排序及申请单跳转。
