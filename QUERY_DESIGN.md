...
`检验结果查询` 页面同样复用通用报告查询链路，但该页面报告列表不展示医嘱内容，因此会通过 `skip_order_text` 跳过 `LS_AS_BARCODE.ORDER_TEXT` 的 `FOR XML PATH` 聚合；`常规报告` 仍展示医嘱内容，继续保留聚合逻辑。主查询、检验结果查询和常规报告的批量 ListView 填充会在 C++ 端用 `WM_SETREDRAW` 暂停重绘，完成所有行列更新后统一刷新；已签收条码查询改用 `LVS_OWNERDATA` 虚拟列表，仅登记结果总行数，并在控件请求可见单元格时返回文本。该模块始终保留全量查询记录，以独立行索引完成排序和条码归组；导出捕获当前索引顺序后在后台逐行写入 UTF-8 BOM CSV，因此界面虚拟化不会截断展示、排序或导出数据。现场 `186,431` 行验证结果为 `sort_ms=41`、`listview_ms=4`，数据库查询及取数为 `10,132 ms`，后续性能分析应继续将数据库阶段和界面阶段分开判断。
