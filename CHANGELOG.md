...
- **已签收条码查询时间与列表优化**：签收日期起止控件改为支持小时分钟，默认当天 `00:00` 至 `23:59`，查询结束条件按 `< DATEADD(minute,1,结束时间)` 处理；顶部签收日期、条形码和病人号输入区域加宽，其他控件保持宽度不变并顺延；下方 ListView 在费用列前新增 `检验者`、`审核者`，复用常规报告口径从最近有效报告的 `OPER_CODE / REP_OPER` 关联 `JC_EMPLOYEE_PROPERTY.NAME`，排序、复制单元格和 CSV 导出同步支持新增列。
- 版本号 v2026.07.06。
