...
- **已签收条码查询优化**：`已签收条码查询` 日期类型默认选中 `签收日期`；条形码、姓名和病人号输入框支持回车直接查询；下方 ListView 支持点击表头对当前已加载结果做本地升降序排序，费用列按数值比较，其余列按文本比较，排序后保留当前选中行，并支持右键复制当前单元格；`导出Excel` 可将当前已加载且已排序的列表导出为 UTF-8 BOM CSV。
- 版本号 v2026.07.05。
