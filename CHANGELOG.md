...
- **大量输血界面简化**：查询条件使用带标题安全区的分组式两行布局，各区块按 DPI 连续计算间距以避免控件重叠，并保持与其他 MDI 模块一致的最大化和标题栏行为；原十列表汇总改为六项关键指标卡片；事件和逐项明细默认隐藏低频技术列但 CSV 继续完整导出；明细范围下拉框改为随统计口径切换的“当前事件血袋/成分、全部血袋/成分、异常核查”页签，并增加当前患者、事件窗口、折算量和计量项上下文。查询完成状态压缩为口径、时间、院区、事件/异常/核查数、原始记录数和耗时，不再重复显示规则版本等筛选信息。
- 版本号 v2026.08.27.3。
