...
- **已签收条码查询院区筛选**：第一行在专业组前新增 `全部 / 老院 / 新院` 院区下拉，并适当缩短条形码、姓名和病人号输入框；按申请科室 `LS_AS_BARCODE.DEPT_NAME` 是否包含“滨水”在 C++ 侧派生院区并过滤；专业组仅加载 `LS_AS_ROOM.Dept_Code=102/401` 的有效记录，并与院区联动显示两院、`102` 或 `401` 的专业组；查询完成状态显示当前院区，CSV 默认文件名同步包含院区。
