...
- **已签收条码查询院区筛选与条码分组显示**：第一行在专业组前新增 `全部 / 老院 / 新院` 院区下拉，并适当缩短条形码、姓名和病人号输入框；按申请科室 `LS_AS_BARCODE.DEPT_NAME` 是否包含“滨水”在 C++ 侧派生院区并过滤；专业组仅加载 `LS_AS_ROOM.Dept_Code=102/401` 的有效记录并与院区联动；查询状态移至按钮行下方、结果列表上方，ListView 释放原底部状态栏空间并向下占满；同一非空条形码的多条医嘱连续成组，仅首行显示样本号至申请科室等公共字段，点击表头排序后重新归组，底层数据和 CSV 保持完整；查询完成状态显示当前院区，CSV 默认文件名同步包含院区。
