...
- **常用电话模块**：工具菜单和自定义工具栏新增 `常用电话`，采用 RTF + RichEdit 内嵌只读查看器加载 `documents` 下的 RTF 文件；页面按 MDI 最大化打开，顶部控件压缩为单行布局，移除可见的 `查找下一个` 和 `打开目录` 按钮，支持上传 RTF、下拉选择多个常用电话文件并持久化当前选择；控件字体跟随系统设置字号，搜索框作为主输入焦点，输入变化时立即自动搜索，输入停止 1000ms 后全选搜索词，`Enter` / `F3` 可继续查找下一个；支持刷新重载，去除“已加载”等状态提示。
- 版本号 v2026.07.02。
