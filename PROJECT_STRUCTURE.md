...
| `barcode_module.cpp/h` | 已签收条码查询单实例 MDI 子窗口，按 `LS_AS_BARCODE` 只读检索；日期类型默认 `签收日期`，起止控件支持小时分钟并默认当天 `00:00` 至 `23:59`；条形码、姓名和病人号输入框支持回车直接查询；专业组筛选位于第一行上机状态前；列表在费用列前展示最近有效报告的检验者和审核者，人员名称复用 `LS_AS_REPORT.OPER_CODE / REP_OPER -> JC_EMPLOYEE_PROPERTY.NAME` 口径；上机状态通过 `LS_AS_REPORT.REP_NO / CHK_FLAG / CONF` 优先校正，`LS_AS_BARCODE.OPER_STATE` 仅作已签收未上机/已上机兜底，避免条码状态更新滞后；按钮区右侧状态图例用横向小色块加文字对应列表行背景色；后台查询期间保留上一轮列表，查询成功后暂停 ListView 重绘并整体替换结果，减少慢查询期间空白闪烁；结果列表支持点击表头对当前内存结果做本地升降序排序，页面不再提供独立排序下拉框，费用列按数值比较并保留当前选中行，支持右键复制当前单元格，双击已上机条码可复用常规报告跳转消息定位对应报告；`导出Excel` 将当前内存列表按显示顺序导出为 UTF-8 BOM CSV |
