...
| `barcode_module.cpp/h` | 已签收条码查询单实例 MDI 子窗口，按 `LS_AS_BARCODE` 只读检索；日期类型默认 `签收日期`，条形码、姓名和病人号输入框支持回车直接查询；后台查询期间保留上一轮列表，查询成功后暂停 ListView 重绘并整体替换结果，减少慢查询期间空白闪烁；结果列表支持点击表头对当前内存结果做本地升降序排序，费用列按数值比较并保留当前选中行，支持右键复制当前单元格；`导出Excel` 将当前内存列表按显示顺序导出为 UTF-8 BOM CSV |
