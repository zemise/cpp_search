...
| `barcode_module.cpp/h` | 已签收条码查询单实例 MDI 子窗口，按 `LS_AS_BARCODE` 只读检索；日期类型默认 `签收日期`，起止控件支持小时分钟并默认当天 `00:00` 至 `23:59`；第一行依次显示日期范围、条形码、姓名、病人号、院区、专业组和上机状态，文本框支持回车查询；院区按申请科室是否包含“滨水”派生，专业组仅加载 `LS_AS_ROOM.Dept_Code IN (102,401)` 的有效记录，并按全部、老院 `102`、新院 `401` 联动显示；专业组和上机状态均使用原生 ComboBox 主题外观的多选下拉，支持鼠标点击、`F4` 和 `Alt+↓` 打开，并按屏幕工作区限位；未完成检验通过上机状态多选 `已签收未上机 / 已上机未审核 / 已审核未发送` 自定义查询；列表展示最近有效报告的检验者和审核者，上机状态通过 `LS_AS_REPORT.REP_NO / CHK_FLAG / CONF` 优先校正；按钮区右侧状态图例对应列表行背景色；后台查询成功后整体替换列表；结果支持表头本地排序、右键复制、双击跳转常规报告和按当前顺序导出带院区文件名的 UTF-8 BOM CSV |
