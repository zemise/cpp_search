...
| `barcode_label_printing.cpp/h` | Win32 主程序条码标签打印共享 helper，统一读取 `[RegularReport] BarcodePrinterName`，封装 LabelPrint 打印调用；常规报告和标本签收中心共用；检测到 Zebra/ZD888t 时复用 LabelPrint Zebra 测试打印的默认布局并只使用 `E:SIMSUN.TTF` 输出中文，避免 `E:CSONG.TTF` fallback 叠印 |
