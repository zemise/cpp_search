...
- 工具菜单中的 `已签收条码查询` 已接入 `LS_AS_BARCODE` 只读查询：
  ...
  - 同一非空条形码对应多条医嘱时，ListView 将这些记录连续成组，仅第一行显示样本号、急诊、条形码、病人号、类型、姓名、性别和申请科室，后续医嘱行的这 8 列留空；底层行数据保持完整。点击任意表头排序后会按排序结果重新组织条码组，并继续保证每组第一行显示完整信息；CSV 仍导出每一行的完整字段。
  - 下方列表支持点击表头对当前已加载结果做本地升降序排序，不重新查询数据库；页面不再提供独立排序下拉框，费用和签收-审核时间差按数值排序，其余列按文本排序，排序后保留当前选中行；右键菜单按 `复制：实际内容` 显示当前单元格预览，长文本在菜单中省略但复制值保持完整；双击已上机条码行可跳转到 `常规报告` 并定位对应报告，已签收未上机条码会提示无法跳转。
  - `导出Excel` 会将当前已加载且已排序的列表导出为 UTF-8 BOM CSV，不重新查询数据库，默认文件名包含当前院区；取消签收、取消医嘱签收、取消原因限制保持禁用，不执行数据库修改。
