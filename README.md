...
- 工具菜单中的 `已签收条码查询` 已接入 `LS_AS_BARCODE` 只读查询：
  ...
  - 支持按条形码、姓名、病人号、上机状态、专业组、取消签收状态筛选；条形码、姓名和病人号输入框支持回车直接查询；取消签收状态按 `CANCEL_DATE` 是否为空判断，上机状态采用报告链路优先校正：`LS_AS_REPORT.CONF='S'` 显示发送完成，`CHK_FLAG='T'` 且未发送显示已审核未发送，有报告号或 `LS_AS_BARCODE.OPER_STATE>=1` 显示已上机未审核，否则按已签收未上机显示。
  - 按钮区右侧状态图例采用横向小色块加文字展示，与列表行背景色保持一致。
  - 下方列表展示样本号、急诊、条形码、病人号、类型、姓名、性别、申请科室、床号、签收人、签收时间、医嘱内容、标本、检验者、审核者、费用、申请医生、状态、备注、原因、送检、送检时间、申请时间、取消时间、取消人、HZID、上机状态；检验者和审核者复用常规报告的人员字典口径，分别由最近有效报告的 `OPER_CODE`、`REP_OPER` 关联 `JC_EMPLOYEE_PROPERTY.NAME`。
