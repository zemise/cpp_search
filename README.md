...
- 底部 `趋势图` 按钮可按上一次成功查询条件打开趋势窗口：
  ...
  - 下方显示趋势明细表，便于和主界面核对。
  - 右侧底部 `导出勾选项目` 可将勾选项目的趋势明细导出为 Excel `.xlsx` 工作簿。
  - 右侧底部 `导出勾选图片` 可选择一个文件夹，并将勾选项目分别导出为多张 PNG。
  - 导出默认文件名为 `病人姓名-病人号-查询日期.xlsx`，病人姓名或病人号为空时自动跳过对应部分。
...
- 系统设置支持选择常规报告条码打印机，保存到 `ClientConfig.ini` 的 `[RegularReport] BarcodePrinterName`；条码打印机区域的 `Zebra 字体` 下拉框仅在识别到 Zebra/ZD888t 时生效，保存到 `[RegularReport] ZebraChineseFont`，可在 `E:SIMSUN.TTF` 与 `E:CSONG.TTF` 间选择。
...
- 主程序自定义工具栏提供 `标本签收中心`、`常规报告`、`输血查询`、`结果查询`、`质控分析`、`门诊查询`、`常用电话` 快捷入口；其中 `标本签收中心` 和 `常规报告` 只保留在自定义工具栏，不再重复出现在 `工具` 菜单中；活动 MDI 页面会高亮对应入口，右侧 `关闭当前` 为文字型次要操作按钮，用于关闭当前 MDI 子窗口。
- `工具` 菜单当前保留 `已签收条码查询(&1)`、`脂血MCHC校正(&2)`、`门诊查询(&3)` 和 `常用电话(&4)`。
- 工具菜单中的 `已签收条码查询` 已接入 `LS_AS_BARCODE` 只读查询：
  - 默认不自动查询，日期类型默认 `签收日期`，日期时间范围默认当天 `00:00` 至 `23:59`，支持切换申请日期、上机日期。
  - 第一行筛选顺序为日期范围、条形码、姓名、病人号、院区、专业组和上机状态；条形码、姓名和病人号输入框支持回车直接查询，第二行提供取消签收状态和操作按钮，查询状态显示在按钮行下方、结果列表上方，采用左对齐且左边缘与“查询”按钮左边缘对齐；列表不再为窗口底部状态栏预留空间。
  - 院区提供 `全部 / 老院 / 新院`，按申请科室 `LS_AS_BARCODE.DEPT_NAME` 是否包含“滨水”派生新院或老院。专业组来源 `LS_AS_ROOM`，仅加载 `DELETE_BIT=0` 且 `Dept_Code IN (102,401)` 的记录，并随院区联动：全部显示两院专业组，老院只显示 `Dept_Code=102`，新院只显示 `Dept_Code=401`；切换院区会清理当前院区不可用的专业组选择。
  - 专业组和上机状态使用原生 ComboBox 主题外观的下拉按钮，弹出层为带复选框的 ListView，可选择 `全部` 或多项具体条件，并支持 `F4` / `Alt+↓` 打开；取消签收状态按 `CANCEL_DATE` 是否为空判断，上机状态采用报告链路优先校正：`LS_AS_REPORT.CONF='S'` 显示发送完成，`CHK_FLAG='T'` 且未发送显示已审核未发送，有报告号或 `LS_AS_BARCODE.OPER_STATE>=1` 显示已上机未审核，否则按已签收未上机显示。
  - 未完成检验不再作为独立按钮；需要查询未完成时，在主 `上机状态` 下拉中勾选 `已签收未上机`、`已上机未审核`、`已审核未发送`，并可同时用主 `专业组` 下拉多选限定专业组。
  - 按钮区右侧状态图例采用横向小色块加文字展示，与列表行背景色保持一致。
  - 下方列表删除原“样本号”前的空白占位列，依次展示样本号、急诊、条形码、病人号、类型、姓名、性别、申请科室、床号、签收人、签收时间、医嘱内容、标本、检验者、审核者、审核时间、签收-审核时间差、上机状态、费用、申请医生、状态、备注、原因、送检、送检时间、申请时间、取消时间、取消人和 HZID；签收人直接显示 `LS_AS_BARCODE.OPER_CODE`；检验者和审核者复用常规报告的人员字典口径，主查询只返回最近有效报告的 `OPER_CODE`、`REP_OPER`，`JC_EMPLOYEE_PROPERTY` 字典按数据库连接独立加载并在进程内缓存，由 C++ 映射 `NAME`，字典加载失败或代码缺失时回退显示原值，避免人员表参与大结果集查询；审核时间显示该报告的 `LS_AS_REPORT.REP_TIME`；签收-审核时间差在后台线程中由 C++ 通过无时区公历算术计算到秒，相同时间组合复用缓存，不增加数据库计算负荷或阻塞 UI 线程。
  - 同一非空条形码对应多条医嘱时，ListView 将这些记录连续成组，仅第一行显示样本号、急诊、条形码、病人号、类型、姓名、性别和申请科室，后续医嘱行的这 8 列留空；底层行数据保持完整。列表使用 `LVS_OWNERDATA` 虚拟模式，只向控件登记总行数并按当前可见区域提供文本，避免大结果集逐行、逐单元格填充；点击任意表头后仅重排轻量行索引并重新组织条码组，不搬动完整记录，且继续保证每组第一行显示完整信息。现场 `186,431` 行验证中，索引排序为 `41 ms`、列表绑定为 `4 ms`，改造前同量级列表填充为 `31,818 ms`。
  - 下方列表支持点击表头对当前已加载的全部结果做本地升降序排序，不重新查询数据库；页面不再提供独立排序下拉框，费用和签收-审核时间差按数值排序，其余列按文本排序，排序后保留当前选中行；右键菜单按 `复制：实际内容` 显示当前单元格预览，长文本在菜单中省略但复制值保持完整；双击已上机条码行可跳转到 `常规报告` 并定位对应报告，已签收未上机条码会提示无法跳转。
  - `导出Excel` 会将当前已加载且已排序的全部结果导出为真正的 OOXML `.xlsx` 工作簿，不重新查询数据库，默认文件名包含当前院区；所有单元格以 Unicode 文本保存，避免旧版 Excel 打开 UTF-8 CSV 时按本机代码页解释而出现中文乱码，并保留条码、病人号等字段的前导零。导出在线程中按行流式写入临时文件，界面显示进度，成功后再替换目标文件，不一次性构造整份工作簿；超过 Excel 单表行数上限时自动拆分到后续工作表。查询、刷新和排序在导出期间暂时锁定，确保导出快照一致；取消签收、取消医嘱签收、取消原因限制保持禁用，不执行数据库修改。
  - 页面按操作类型提供非阻塞反馈：数据库查询时在列表中央显示白色加载卡片、查询文案和不确定进度条，Excel 导出时复用加载卡片显示已完成行数和确定进度，不再叠加功能重复的 Spinner 或完成 Toast；查询、复制和导出成功信息持续显示在状态行。加载结束时先隐藏进度条再停止动画，随后隐藏文字和卡片并统一重绘，避免出现没有进度条的灰色过渡卡片；普通查询与导出错误显示在状态行红色 Alert 中，用户需要时可点击查看完整错误详情，只有数据库未配置等无法继续的前置问题直接使用 Modal。主要控件均提供 Tooltip；当前没有需要 Drawer 承载的复杂详情，禁用的取消签收写操作也暂不增加 Popconfirm。
- 工具菜单新增 `门诊查询`，位于 `常用电话` 前；页面顶部采用两行筛选布局，并按当前系统字体测量标签宽度，按收费时间默认当天起止、院区、门诊号、姓名和身份证筛选门诊收费明细，院区后提供默认不勾选的 `包含非检验科`，勾选后不再按院区限定 `ZXKS`；门诊号、姓名和身份证输入框均支持回车直接查询；门诊号筛选直接对应 `YJ_MZSQ.BLH`，按末尾匹配查询，便于输入短尾号定位完整门诊号；输入门诊号时若前 8 位可推断 `YYYYMMDD`，则按该日期查，否则只查第二个收费时间控件所在日期；院区 `全部` 仅汇总 `ZXKS=102/401` 的老院和新院数据；主表读取 `YJ_MZSQ.BLH / FPH / SQNR / SQKS / DJ / SL / DW / JE / SFRQ / TXM / BBMC / TXMDYSJ / ZXKS`，并通过 `YJ_MZSQ.BRXXID = YY_BRXX.BRXXID` 读取 `SFZH / BRXM / XB / CSRQ`；身份证筛选对应卡号列字段 `YY_BRXX.SFZH`；列表在项目名称后显示 `申请科室`，对应 `YJ_MZSQ.SQKS` 并按 `JC_DEPT_PROPERTY.DEPT_ID -> NAME` 转换为名称，匹配不到时回退原代码；右键弹出 `复制：实际内容` 菜单并复制完整值；有条码号的记录按 `TXM` 聚合同条码多行 `SQNR`，项目名称用 `/` 去重拼接，无条码号的记录仍逐行显示并用黄色行背景提示；院区按 `ZXKS` 的 `102 / 401` 映射老院/新院，`FPH` 为空时显示 `0`，卡号显示 `SFZH`，`XB` 按 `1/2` 显示男女，`CSRQ` 转换为年龄，`TXM` 为空时条码号显示 `未生成`，只做只读查询。
- 工具菜单新增 `常用电话`，采用 RTF + RichEdit 内嵌只读查看器加载程序目录下 `documents` 目录中的 RTF 文件；页面按 MDI 最大化打开，RichEdit 查看区放在浅边框白色文档卡片内并从顶部占满窗口主体，右上角独立白色控制卡片显示两行操作：上行为搜索框，下行为文档下拉、上传和刷新等低频操作；正常宽度下文档卡片会让出右上角控制卡片宽度，避免两者重合；可上传 RTF、下拉选择多个常用电话文件，并将当前选择持久化到 `ClientConfig.ini` 的 `[PhoneDirectory] SelectedFile`，下次打开保持一致；控件字体跟随系统设置字号，搜索框作为主输入焦点，输入变化时立即自动搜索并保留当前输入内容，`Enter` / `F3` 可继续查找下一个；内容按页面宽度居中显示，搜索跳转按命中行号定位并立即完整刷新，避免 RichEdit 自动滚动到远离命中的位置；滚动条拖动中节流刷新、滚动结束立即完整刷新，以兼顾 Windows 7 实机残影修复和滑块拖动流畅度；支持刷新重载。
- 自定义工具栏中的 `标本签收中心` 已替换原 `工具3` 占位页，当前按 `temp/模版.png` 完成界面骨架，并接入第一版只读条码查询；有条码时按单个条码精确读取数据库并回填页面，左侧条码为空时可按签收日期和/或申请日期查询已签收条码列表，不执行签收、拒签、导出等数据库写入业务操作。下方列表补充签收时间、送检时间、年龄、签收人、标本类型和检验室；`此条码已存在...` 使用独立红色提示标签，普通查询状态显示在左下按钮区，且未上机条码会追加 `未上机检验!`。`补打条码` 复用常规报告页的 LabelPrint 打印入口，样本号固定为 `补`，组合项目固定取当前选中行的 `检验室` 内容；`管理` 按钮会打开或激活 `已签收条码查询` 页面。
- `统计分析管理` 下七个统计模块统一复用公共页面反馈组件：后台查询期间在主明细区域中央显示白色加载卡片和 `PBS_MARQUEE` 不确定进度条，完成或失败后批量隐藏控件并只重绘卡片区域，避免结束瞬间残留空白灰色卡片；查询成功信息持续保留在状态行，查询失败显示红色可点击 Alert，完整错误按需通过 Modal 查看；输入无效、数据库未配置等无法继续执行的前置问题仍直接使用 Modal。查询、导出、关键筛选和明细列表提供 Tooltip；不使用 Spinner 或自动消失的 Toast。
- 统计分析管理菜单新增 `HIV 抗体检测统计` 第一版页面，按 `LS_AS_REPORT.REP_TIME` 月份只读统计三组已确认 HIV 初筛候选项目，只纳入已审核、已发送且姓名非空的报告，按 `REP_NO + MACH_CODE + ITEM_CODE` 去重填充合计行的初筛检测数和初筛阳性数；HIV 明细查询先按月份、审核、发送和仪器从 `LS_AS_REPORT` 筛出候选 `REP_NO`，再按候选 `REP_NO` 分批查询 `LS_AS_REPENTRY` 的目标项目；仪器、病人类型、科室名称字典转为 C++ 内存映射，避免主查询使用大 `OR`、多字典 JOIN 和 `LIKE '%滨水新城%'`；顶部 `新院 / 老院` 筛选会先由 `JC_DEPT_PROPERTY` 分出新院 `DEPT_ID` 集合，选择 `新院` 时下推 `r.DEPT_CODE IN (新院代码...)`，选择 `老院` 时下推 `DEPT_CODE` 为空/`NULL` 或 `r.DEPT_CODE NOT IN (新院代码...)`；`性病门诊` 行暂按下方明细 `科室` 包含 `皮肤科门诊` 的文字口径填充，`其他就诊检测` 行暂按 `科室` 包含 `体检`、`儿童保健`、`健康管理`、`GCP` 或科室为空/`0` 的文字口径填充，`孕产期检查` 行暂按 `科室` 包含 `产科门诊` 或 `早孕关爱门诊` 的文字口径填充，后续再考虑将这三类改为 `DEPT_ID` 精确口径；`受血（制品）前检测` 行按下方明细 `已完结输血单申请号` 非空统计；`术前检测` 行按总数扣除受血、性病门诊、其他就诊和孕产期检查后的剩余量统计；阳性按结果文本判断，`待确认` 优先，其次为 `阳性` 或 `+`，不做结果值与上下限的数值比较；上方样本来源分类表将 `合计` 置顶并用浅蓝背景突出，原 DOCX 统计表导出模板顺序不变；样本来源分类表下方增加独立方法学汇总表，按下方明细行统计 `化学发光法` 和 `酶免法` 的初筛检测数、初筛阳性数；顶部可按 `全部 / 新院 / 老院` 筛选检验科来源；下方明细列表展示仪器、样本来源、方法学、检验科、项目、病人号、姓名、科室、结果和报告时间，其中 `样本来源` 按上方汇总分类口径落单行分类，`方法学` 按 `MACH_CODE` 派生为 `4005/914 -> 化学发光法`、`4008 -> 酶免法`，`病人号` 来自 `LS_AS_REPORT.REG_NO`，`科室` 按 `LS_AS_REPORT.DEPT_CODE -> JC_DEPT_PROPERTY.DEPT_ID -> NAME` 显示，`检验科` 按科室名称是否包含 `滨水新城` 显示 `新院/老院`，`已完结输血单申请号` 先按当前统计月份的 `LS_XK_BloodRequestApply.Apply_Time` 预取当月 `ApplyForm_Statue='已完结'` 申请记录，再按当前明细 `REG_NO` 匹配 `LS_XK_BloodRequestApply.Patient_NO` 的 `ApplyFormNO`；阳性行用红色背景提示，支持点击任意表头对当前已加载明细做本地排序，双击明细行可跳转到 `常规报告` 页面并按目标 `REP_NO` 定位，不额外查询统计数据；`导出统计表` 按钮会基于安装目录 `templates/HIVStatisticsTemplate.docx` 的 `{}` 占位符生成 Word `.docx` 统计表，模版不随项目发布，需通过旁边的 `上传模版` 指定并复制到安装目录；未检测到匹配模版时 `导出统计表` 不可用；`导出明细表` 会将下方当前明细列表按当前排序导出为 Excel `.xlsx` 工作簿，不额外查询数据库；统计表只导出当前汇总数据，非合计行数字为 `0` 时导出为空，合计行保留 `0`。WB 检测数、复检和本年度累计暂未接入。
...
- 统计分析管理菜单新增 `免疫重复项目统计`，入口为 `免疫重复项目统计(&3)`；阶段 1 已完成 Win32 原生 MDI 页面骨架，顶部采用两行筛选布局，第一行显示签收时间范围，第二行显示查询、导出和状态文本，签收时间标签按当前字体与 DPI 动态测量宽度；阶段 2 已接入住院真实只读查询：以 `YJ_ZYSQ.JSSJ` 为签收时间，按 `SQNR` 包含 `输血前常规检查` 识别基准医嘱，按包含 `乙肝三对` 或 `梅毒` 识别疑似重复医嘱，基准和重复查询均强制限定 `ZXKS IN (102,401)`，再按 `INPATIENT_ID` 聚合同一次住院并通过 `ZY_INPATIENT.INPATIENT_NO` 展示病人号；查询在后台线程执行，成功后整体填充汇总和明细，按 `YJSQID` 保证重复申请一行一项。当前不写业务表，Excel 明细导出、基于 `YJ_ZYSQ.TXM -> LS_AS_BARCODE.BARCODE` 的床号回填和 LIS 报告跳转尚未接入。
- 统计分析管理菜单新增 `备血统计`，入口为 `备血统计(&4)`；按申请日期 `LS_XK_BloodRequestApply.Apply_Time` 和可选申请状态只读查询，以 `TranProperty='备血'` 判断申请类型命中，以 `UseBloodNote` 内容包含“备血”判断用血备注命中，以 `Apply_Purpose` 内容包含“备血”判断输血目的命中，不再统计 `UrgencyLevel`；申请状态下拉为全部、未审核、已审核、已完结和已驳回，默认排除 `Delete_Bit=1` 或状态为已删除的申请，可通过“包含已删除”显式纳入；备血类型下拉为全部、申请类型、用血备注、输血目的和多项命中；按去空格后的唯一 `ApplyFormNO` 去重，汇总区显示备血口径与状态分布，同一申请单多处命中时总数只计一次；院区下拉提供全部、老院、新院，院区在 C++ 内存中按 `Apply_Dept` 是否包含“滨水”派生后过滤，再计算汇总、状态分布、明细和空申请单号异常，不向 SQL 增加院区模糊条件；顶部采用两行筛选布局并按当前字体动态测量标签宽度；明细首列及 Excel 工作簿显示院区，整行背景按未审核橙色、已审核蓝色、已完结绿色、已驳回红色、已删除灰色区分，筛选区右侧提供与“已签收条码查询”一致形态的色块图例；支持备血类型内存筛选、点击表头本地排序，并将当前显示顺序导出为带院区后缀的 Excel `.xlsx` 工作簿；双击未删除明细可按申请单号和申请日期跳转到“输血结果查询”并选中对应行，不自动切换右侧页签，已删除明细仅提示且不跳转；模块不写 LIS 业务表。
- `大量输血统计(&5)` 已正式默认“实际输血量”，默认事件时间为“出库时间”，并保留“申请量对照”供主动切换。实际口径以 `LS_XK_BloodCrossMatch.VerifyState='已审核'` 为输血事实，按唯一 `BloodInID` 关联 `BloodInfo -> CompositionInfo.Norm + Unit + CompositionTypeID` 计算每袋容量并分类；支持配血、出库、申请、血库审核四种事件时间，同一病人按所选时间划分互不重叠的24小时事件。实际查询按四个时间来源分别生成候选交叉配血 ID，去重后只关联候选事实并聚合候选血袋的出库记录，避免跨表 `OR + COALESCE`、逐行相关聚合和连接列函数；C++ 再按原时间优先级精确复核范围，查询状态显示原始记录数与耗时。未审核、未知状态、交叉配血逻辑删除、重复血袋、时间/容量缺失进入异常核查；申请单删除、驳回、缺失或病人号不一致不覆盖实际输血事实，仍计量并标记。折算规则 `v4` 以 `CompositionTypeID / CompositionBig_ID` 分类，按 `ML×1 / 普通U×200 / 类型3冷沉淀U×20 / 治疗量×250` 换算；制品名称仅展示、不参与分类或兜底，血小板和冷沉淀默认排除，阈值默认 `>=1600ml` 且可修改。事件列表与 Excel 工作簿在实际口径下明确显示“实际输血制品构成”，其内容只按实际血袋成分和折算量汇总；申请量对照则显示“申请制品构成”。页面采用查询条件分组、六项关键指标卡片、精简默认列和动态明细页签，并在明细区固定显示当前患者及事件窗口；隐藏的技术字段仍完整导出。模块只读，不写 LIS 业务表。完整口径见 `MASSIVE_TRANSFUSION_STATISTICS_DESIGN.md`。
- 统计分析管理菜单新增 `输血单统计`，入口为 `输血单统计(&6)`；仅按申请日期 `LS_XK_BloodRequestApply.Apply_Time` 只读查询申请主表，按去空格后的唯一 `ApplyFormNO` 去重，并根据 `ApplyForm_Statue` 汇总未审核、已审核和已完结申请单；已驳回和已删除分别由默认不勾选的复选框控制，`Delete_Bit=1` 始终按已删除归类，未知状态不进入正式总数并计入其他状态异常；院区下拉提供全部、老院、新院，按 `Apply_Dept` 是否包含“滨水”在 C++ 内存中派生过滤后重新计算汇总；页面提供两组状态汇总、完整申请明细、状态行配色、表头本地排序、Excel `.xlsx` 导出及未删除申请单双击跳转，查询失败保留上一轮结果，不关联申请成分、交叉配血或出库表，不写 LIS 业务表。
- 统计分析管理菜单新增 `检验周转时间统计(&7)`：按 `LS_AS_BARCODE.IN_DATE` 查询已签收条码并按唯一条码聚合，专业组、申请科室、医嘱项目、住院/门诊及急诊条件可筛选；采集时间只读取 `COLLECTION_TIME`，为空不回退 `SUB_DATE`，上机时间复用常规报告的 `LS_AS_REPORT.CREATE_TIME`，发送时间按已签收条码查询口径等同审核时间 `LS_AS_REPORT.REP_TIME`。页面计算采集-接收、接收-上机、接收-发送和采集-发送四段秒级 TAT，默认阈值为 `30 / 30 / 180 / 240` 分钟，可在独立阈值弹窗中修改并保存到本机配置；任一阶段超过阈值即整条超时，未上机或未审核阶段按当前时间持续计算等待时长并每分钟刷新。流程状态沿用已签收条码查询的报告链路优先规则，明细支持正常/超时/时间异常筛选、行配色、本地排序、单元格复制、Excel `.xlsx` 导出和常规报告跳转；未签收、周期配置、计算信息、往期信息和报表打印暂缓并预留入口，模块不写 LIS 业务表。
- 统计分析管理菜单新增 `质控分析` 页面：用户先在系统设置中维护某台检验仪器的固定质控样本号和项目级质控名称/水平；质控品设置区采用左侧配置列表、右侧双列表单，读取日期使用日历选择器且 `读取项目` 放在同一行，批号开始日期和结束日期也使用日历选择器，其中结束日期允许为空；可按 `仪器 + 样本号 + 指定日期` 只读读取 LIS 项目清单并保存为本机质控项目，重复读取会复用已有样本配置并更新本机项目清单；若右侧质控名称、水平、批号、日期、靶值、SD 或备注非空，会批量覆盖到本次读取到的项目，之后仍可选中单个项目行单独调整。质控批号按样本号维护开始日期、可空结束日期，以及该批号下各项目独立靶值和 SD；质控分析页从右侧操作侧栏选择日期、仪器、水平和状态后，`查询` 只读取本机 SQLite 中已同步的质控结果镜像并套用当前批号/靶值配置重新分析，`导入质控` 会弹出日期范围选择，默认当天；确认后按所选日期范围和已配置质控样本号只读拉取 LIS 明细，并覆盖更新 `qc_result_cache / qc_query_cache_meta`，但不会改动主页面日期筛选或自动刷新当前卡片。页面在内存中按结果日期匹配当时生效的本机批号和项目靶值，完成分组、均值/SD/CV%、Z 值和 Westgard 规则判断，已覆盖 `1-2s / 1-3s / 2-2s / R-4s / 4-1s / 10x`；主区域采用卡片优先看板，摘要条按项目统计失控、警告、在控、无数据和未判定数量，卡片标题按 `（项目英文名）项目名` 显示，正文显示批号、靶值和操作人，未维护靶值时显示“均值”；卡片标题栏单击打开 L-J 图，正文单击只选择卡片；右侧侧栏提供 `L-J图 / 明细 / 导出` 和项目列表，底部明细默认收起并可按需展开；L-J 图支持多水平纵向堆叠、滚动、点位 tooltip、点击图点定位明细和明细高亮图点，并在维护靶值后以靶值作为中心线；状态栏、明细来源列和 Excel 导出会标识 `LIS导入` 或 `本机缓存`。质控样本、项目、批号、项目靶值和本机质控结果镜像保存在 SQLite，质控结果事实来源以 LIS 为准且不写 LIS。
- 自定义工具栏中的 `常规报告` 已替换原 `工具2` 占位页，按 `temp/模版2.png` 基本完成三栏报告工作台界面：
  ...
  - 右侧信息列表第一列支持勾选；行右键菜单提供 `打印条码`、`打印勾选条码` 和 `趋势图`。条码打印会把对应行样本号、组合项目、条码号、姓名、标本、开单日期、科室代码、病人号填入 LabelPrint 的 `MedicalLabelData`，再交给共享条码打印 helper 发送到条码打印机；其中条码上的组合项目取自该右侧报告行的 `检验仪器` 列内容，开单日期按 `2026/5/21` 这种仅日期格式打印。右键 `趋势图` 和底部 `趋势图` 按钮复用检验结果查询页趋势图窗口，按当前报告的病人号/姓名、当前检验仪器和科室生成查询条件，窗口顶部提供开始/结束日期选择器和刷新按钮，默认结束日期为当前报告检验日期，开始日期为前 14 天；窗口按父窗口所在显示器工作区居中打开。
  - 条码打印机名读取 `ClientConfig.ini` 的 `[RegularReport] BarcodePrinterName`，默认 `Xprinter XP-360B #2`；打印机型号和 TSPL/ZPL 后端由 LabelPrint 根据 Windows 打印机元数据自动选择，识别不出时按 XP-360B 兼容路径兜底。检测到 Zebra/ZD888t 时，本项目复用 LabelPrint Zebra 测试打印的默认布局，并按 `[RegularReport] ZebraChineseFont` 选择 `E:SIMSUN.TTF` 或 `E:CSONG.TTF` 输出中文，默认 `E:SIMSUN.TTF`；仍清空 fallback，避免两种字体叠印导致文字显示不全；Zebra 路径下标签 `组合项目` 按条码水平区域居中显示。如果打印机名失效，需要在系统设置页重新选择条码打印机。
...
- 主程序 `输血结果查询` 模块：
  ...
  - `申请状态` 直接对应 `LS_XK_BloodRequestApply.ApplyForm_Statue` 的中文值，已确认包括“未审核 / 已审核 / 已完结 / 已驳回 / 已删除”；删除范围还需结合 `Delete_Bit` 判断。
  ...
  - `查询检验结果` 窗口可按当前病人号、姓名或身份证查询 LIS 结果；已确认输血申请 `Patient_NO` 与 `LS_AS_REPORT.REG_NO` 同口径，按病人号查询时报告列表和摘要均直接过滤 `LS_AS_REPORT.REG_NO`；按名字查询时会先用当前病人号获取 `LS_AS_REPORT.PAT_PHONE`，取到电话后追加姓名+电话约束以减少重名误匹配；按身份证查询时会从住院病人信息表 `ZY_INPATIENT` 用当前病人号 `INPATIENT_NO` 找到 `SOCIAL_NO`，再查询同一身份证下的所有住院号并过滤 `LS_AS_REPORT.REG_NO`；检验摘要区下方会显示身份匹配可信度提示；摘要区根据可配置项目代码显示最近一次血型鉴定、血红蛋白/血小板、不规则抗体筛查和直接抗人球蛋白试验摘要，新项目显示为“不规则”和“直抗”两行。
  ...
  - `查询检验结果` 窗口的报告列表使用专用轻量 SQL，仅读取弹窗展示和选择所需字段，并按 `BloodLisExcludeMachines` 下推排除不想展示的科室/仪器；组合项目列表和摘要信息分别走独立后台查询，组合项目列表不等待摘要查询完成；右侧详情列表的 `参考范围` 按下限在前、上限在后显示为 `下限~上限`。
...
12. 在趋势图窗口右侧点击项目行切换图表，勾选项目后可导出对应趋势明细 Excel 工作簿。
