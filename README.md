...
- 工具菜单中的 `已签收条码查询` 已接入 `LS_AS_BARCODE` 只读查询：
  ...
  - 支持按条形码、姓名、病人号、上机状态、专业组、取消签收状态筛选；条形码、姓名和病人号输入框支持回车直接查询；专业组和上机状态使用原生 ComboBox 主题外观的下拉按钮，弹出层仍为带复选框的 ListView，可选择 `全部` 或多项具体条件，并支持 `F4` / `Alt+↓` 打开；取消签收状态按 `CANCEL_DATE` 是否为空判断，上机状态采用报告链路优先校正：`LS_AS_REPORT.CONF='S'` 显示发送完成，`CHK_FLAG='T'` 且未发送显示已审核未发送，有报告号或 `LS_AS_BARCODE.OPER_STATE>=1` 显示已上机未审核，否则按已签收未上机显示。
  ...
  - `导出Excel` 会将当前已加载且已排序的列表导出为 UTF-8 BOM CSV，不重新查询数据库；取消签收、取消医嘱签收、取消原因限制保持禁用，不执行数据库修改。
