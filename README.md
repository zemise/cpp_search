...
- 工具菜单中的 `已签收条码查询` 已接入 `LS_AS_BARCODE` 只读查询：
  ...
  - 下方列表展示样本号、急诊、条形码、病人号、类型、姓名、性别、申请科室、床号、签收人、签收时间、医嘱内容、标本、检验者、审核者、费用、申请医生、状态、备注、原因、送检、送检时间、申请时间、取消时间、取消人、HZID、上机状态；签收人直接显示 `LS_AS_BARCODE.OPER_CODE`；检验者和审核者复用常规报告的人员字典口径，分别由最近有效报告的 `OPER_CODE`、`REP_OPER` 关联 `JC_EMPLOYEE_PROPERTY.NAME`，字典缺失时回退显示原值。
