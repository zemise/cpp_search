...
- 工具菜单新增 `常用电话`，采用 RTF + RichEdit 内嵌只读查看器加载程序目录下 `documents` 目录中的 RTF 文件；页面按 MDI 最大化打开，顶部控件为单行布局，文档下拉保持紧凑，可上传 RTF、下拉选择多个常用电话文件，并将当前选择持久化到 `ClientConfig.ini` 的 `[PhoneDirectory] SelectedFile`，下次打开保持一致；控件字体跟随系统设置字号，搜索框作为主输入焦点，输入变化时立即自动搜索，输入停止 1000ms 后全选搜索词，下一次可直接覆盖输入，`Enter` / `F3` 可继续查找下一个；支持刷新重载。
