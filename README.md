...
- `统计分析管理` 下六个统计模块统一复用公共页面反馈组件：后台查询期间在主明细区域中央显示白色加载卡片和 `PBS_MARQUEE` 不确定进度条，完成或失败后批量隐藏控件并只重绘卡片区域，避免结束瞬间残留空白灰色卡片；查询成功信息持续保留在状态行，查询失败显示红色可点击 Alert，完整错误按需通过 Modal 查看；输入无效、数据库未配置等无法继续执行的前置问题仍直接使用 Modal。查询、导出、关键筛选和明细列表提供 Tooltip；不使用 Spinner 或自动消失的 Toast。
...
- `大量输血统计(&5)` 已正式默认“实际输血量”，默认事件时间为“出库时间”，并保留“申请量对照”供主动切换。实际口径以 `LS_XK_BloodCrossMatch.VerifyState='已审核'` 为输血事实，按唯一 `BloodInID` 关联 `BloodInfo -> CompositionInfo.Norm + Unit` 计算每袋容量；支持配血、出库、申请、血库审核四种事件时间，同一病人按所选时间划分互不重叠的24小时事件。实际查询按四个时间来源分别生成候选交叉配血 ID，去重后只关联候选事实并聚合候选血袋的出库记录，避免跨表 `OR + COALESCE`、逐行相关聚合和连接列函数；C++ 再按原时间优先级精确复核范围，查询状态显示原始记录数与耗时。未审核、未知状态、交叉配血逻辑删除、重复血袋、时间/容量缺失进入异常核查；申请单删除、驳回、缺失或病人号不一致不覆盖实际输血事实，仍计量并标记。折算规则 `v3` 为 `ML×1 / 普通U×200 / 冷沉淀U×20 / 治疗量×250`；血小板和冷沉淀默认排除，阈值默认 `>=1600ml` 且可修改。事件列表与 Excel 工作簿在实际口径下明确显示“实际输血制品构成”，其内容只按实际血袋成分和折算量汇总；申请量对照则显示“申请制品构成”。页面采用查询条件分组、六项关键指标卡片、精简默认列和动态明细页签，并在明细区固定显示当前患者及事件窗口；隐藏的技术字段仍完整导出。模块只读，不写 LIS 业务表。完整口径见 `MASSIVE_TRANSFUSION_STATISTICS_DESIGN.md`。
