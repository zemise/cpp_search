...
- `大量输血统计(&5)` 已正式默认“实际输血量”，默认事件时间为“出库时间”，并保留“申请量对照”供主动切换。实际口径以 `LS_XK_BloodCrossMatch.VerifyState='已审核'` 为输血事实，按唯一 `BloodInID` 关联 `BloodInfo -> CompositionInfo.Norm + Unit + CompositionTypeID` 计算每袋容量并分类；支持配血、出库、申请、血库审核四种事件时间，同一病人按所选时间划分互不重叠的24小时事件。实际查询按四个时间来源分别生成候选交叉配血 ID，去重后只关联候选事实并聚合候选血袋的出库记录，避免跨表 `OR + COALESCE`、逐行相关聚合和连接列函数；C++ 再按原时间优先级精确复核范围，查询状态显示原始记录数与耗时。未审核、未知状态、交叉配血逻辑删除、重复血袋、时间/容量缺失进入异常核查；申请单删除、驳回、缺失或病人号不一致不覆盖实际输血事实，仍计量并标记。事件仍只按 `Patient_NO` 归并；主列表“姓名”显示事件内最后出现的非空姓名，旁边“姓名数”显示去重姓名数量；该提示不改变统计量、完整性或异常数，逐袋明细保留原始姓名。折算规则 `v4` 以 `CompositionTypeID / CompositionBig_ID` 分类，按 `ML×1 / 普通U×200 / 类型3冷沉淀U×20 / 治疗量×250` 换算；制品名称仅展示、不参与分类或兜底，血小板和冷沉淀默认排除，阈值默认 `>=1600ml` 且可修改。事件列表与 Excel 工作簿在实际口径下明确显示“实际输血制品构成”，其内容只按实际血袋成分和折算量汇总；申请量对照则显示“申请制品构成”。事件 Excel 同时导出姓名、事件内全部姓名、姓名数和是否多姓名。页面采用查询条件分组、六项关键指标卡片和动态明细页签；事件表、实际血袋明细、申请成分明细默认分别精简为 9、8、7 个核心字段，可通过“完整视图”即时恢复全部业务和技术列。简洁视图仅填充可见单元格，事件数值列按数值排序并在排序后保持原选中事件；第二张列表上方按“最后姓名（其他姓名） | 病人号 | 折算量 | 袋数/项数”紧凑显示，Excel 始终完整导出。模块只读，不写 LIS 业务表。完整口径见 `MASSIVE_TRANSFUSION_STATISTICS_DESIGN.md`。
- 统计分析管理菜单新增 `输血单统计`，入口为 `输血单统计(&6)`；仅按申请日期 `LS_XK_BloodRequestApply.Apply_Time` 只读查询申请主表，按去空格后的唯一 `ApplyFormNO` 去重，并根据 `ApplyForm_Statue` 汇总未审核、已审核和已完结申请单；已驳回和已删除分别由默认不勾选的复选框控制，`Delete_Bit=1` 始终按已删除归类，未知状态不进入正式总数；院区下拉提供全部、老院、新院，按 `Apply_Dept` 是否包含“滨水”在 C++ 内存中派生过滤后重新计算汇总；下方明细在“院区”后增加“紧急程度”列，复用“输血结果查询”左侧列表的 `TranProperty` 显示口径，值为 `紧急(电话联系输血科)` 时仅该单元格显示红色背景；在“申请状态”后增加“原因”列，读取 `Remark` 展示驳回原因，两列均同步进入 Excel `.xlsx` 导出；页面还提供分组汇总、状态行配色、表头本地排序及未删除申请单双击跳转，查询失败保留上一轮结果，不关联申请成分、交叉配血或出库表，不写 LIS 业务表。
  - 唯一汇总 ListView 合并展示总数、五种申请状态及紧急、常规、备血三类数量，上方用“总体 / 按申请状态分类 / 按紧急程度分类”自绘分组标题区分两种统计维度；分组标题和数据单元格分别使用浅灰、浅蓝、浅橙同色系背景，“紧急”数值使用红色文字。页面不显示分类合计提示和异常汇总 ListView。
