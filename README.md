...
- 工具菜单中的 `已签收条码查询` 已接入 `LS_AS_BARCODE` 只读查询：
  ...
  - 下方列表删除原“样本号”前的空白占位列，依次展示样本号、急诊、条形码、病人号、类型、姓名、性别、申请科室、床号、签收人、签收时间、医嘱内容、标本、检验者、审核者、审核时间、签收-审核时间差、上机状态、费用、申请医生、状态、备注、原因、送检、送检时间、申请时间、取消时间、取消人和 HZID；签收人直接显示 `LS_AS_BARCODE.OPER_CODE`；检验者和审核者复用常规报告的人员字典口径，分别由最近有效报告的 `OPER_CODE`、`REP_OPER` 关联 `JC_EMPLOYEE_PROPERTY.NAME`，字典缺失时回退显示原值；审核时间显示该报告的 `LS_AS_REPORT.REP_TIME`；签收-审核时间差由 C++ 根据两个时间计算到秒，不增加数据库计算负荷。
  ...
  - 下方列表支持点击表头对当前已加载结果做本地升降序排序，不重新查询数据库；页面不再提供独立排序下拉框，费用列按数值排序，其余列按文本排序，排序后保留当前选中行；右键菜单按 `复制：实际内容` 显示当前单元格预览，长文本在菜单中省略但复制值保持完整；双击已上机条码行可跳转到 `常规报告` 并定位对应报告，已签收未上机条码会提示无法跳转。
