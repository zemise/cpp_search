...
- 工具菜单中的 `已签收条码查询` 已接入 `LS_AS_BARCODE` 只读查询：
  ...
  - 支持按条形码、姓名、病人号、上机状态、专业组、取消签收状态筛选；条形码、姓名和病人号输入框支持回车直接查询；取消签收状态按 `CANCEL_DATE` 是否为空判断，上机状态直接对应 `LS_AS_BARCODE.OPER_STATE`，列表中 `0/1/2` 简化显示为 `未上机 / 已上机 / 审核完成`，下拉中的 `已审核未发送` 暂时不关联实际状态。
  - 下方列表展示样本号、急诊、条形码、病人号、类型、姓名、性别、申请科室、床号、签收人、签收时间、医嘱内容、标本、费用、申请医生、状态、备注、原因、送检、送检时间、申请时间、取消时间、取消人、HZID、上机状态。
  - 下方列表支持点击表头对当前已加载结果做本地升降序排序，不重新查询数据库；费用列按数值排序，其余列按文本排序，排序后保留当前选中行；支持右键复制当前单元格。
