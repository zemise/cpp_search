...
- 统计分析管理菜单新增 `大量输血统计`，入口为 `大量输血统计(&5)`；按事件第一张有效输血申请的 `LS_XK_BloodRequestApply.Apply_Time` 日期归属，只读取页面开始日期及以后数据，并向结束日期后延 24 小时补齐最后一天窗口；按去空格后的同一 `Patient_NO` 划分互不重叠的 24 小时事件，当前事件结束后下一张有效申请建立新事件；申请成分只读取 `LS_XK_BloodRequestApplySon`，不使用主表冗余成分回退；规则 `v2` 按 `ML×1 / 普通U×200 / 冷沉淀U×20 / 治疗量×250` 折算，页面提供默认不勾选的“包括血小板和冷沉淀”，按成分名称包含“血小板”或“冷沉淀”识别，排除项仍显示折算明细但不参与累计；统计条件默认 `>=1600ml`，用户可输入大于 `0` 且最多两位小数的阈值，并在“`>=`/`>`”之间切换，实际条件随查询状态和两级 CSV 输出；未审核、已审核和已完结参与统计，已删除完全排除，已驳回展示但不建立事件、不贡献申请量；院区按首张有效申请的 `Apply_Dept` 是否包含“滨水”派生新院/老院；页面提供事件汇总、24小时事件列表、当前/全部事件成分和独立已驳回核查视图，支持事件与成分两级 UTF-8 BOM CSV、事件表头本地排序，双击事件或成分可跳转到输血结果查询；查询失败保留上一轮结果，模块不写 LIS 业务表。
