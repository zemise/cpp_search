...
- 工具菜单中的 `已签收条码查询` 已接入 `LS_AS_BARCODE` 只读查询：
  ...
  - `导出Excel` 会将当前已加载且已排序的全部结果导出为 UTF-8 BOM CSV，不重新查询数据库，默认文件名包含当前院区；导出在线程中按行流式写入临时文件，界面显示进度，成功后再替换目标文件，避免一次性构造整份 CSV 占用大量内存或长时间阻塞界面。查询、刷新和排序在导出期间暂时锁定，确保导出快照一致；取消签收、取消医嘱签收、取消原因限制保持禁用，不执行数据库修改。
  - 页面按操作类型提供非阻塞反馈：数据库查询时在列表中央显示白色加载卡片、查询文案和不确定进度条，CSV 导出时复用加载卡片显示已完成行数和确定进度，不再叠加功能重复的 Spinner 或完成 Toast；查询、复制和导出成功信息持续显示在状态行。加载结束时先隐藏进度条再停止动画，随后隐藏文字和卡片并统一重绘，避免出现没有进度条的灰色过渡卡片；普通查询与导出错误显示在状态行红色 Alert 中，用户需要时可点击查看完整错误详情，只有数据库未配置等无法继续的前置问题直接使用 Modal。主要控件均提供 Tooltip；当前没有需要 Drawer 承载的复杂详情，禁用的取消签收写操作也暂不增加 Popconfirm。
