...
- 参考模块：页面结构、申请状态口径、删除处理、院区筛选、明细和 Excel 导出主要参照 `备血统计`。
...
5. 按 `TranProperty` 汇总紧急、常规和备血申请单数量，并突出显示紧急申请。
6. 在明细中展示 `Remark` 驳回原因。
7. 提供申请单明细、表头排序、Excel 导出和输血结果查询跳转。
...
- `TranProperty` 已确认使用 `紧急(电话联系输血科) / 常规 / 备血` 三类业务值。
- `Remark` 可直接提供已驳回申请单的原因。
- `Apply_Dept` 已用于现有输血统计的院区派生。
- `Delete_Bit` 已用于现有模块的删除过滤。
- 备血统计已经实现申请单去重、状态分布、院区过滤、Excel 导出和申请单跳转，可复用其实现模式。
...
| `TranProperty` | “紧急程度”和“申请类型”展示来源；紧急值用于单元格着色 |
...
其他未知状态不进入正式总数，仅保留底层异常计数。若后续确认新的有效状态，应更新本文档和程序映射。
...
| 紧急 | `TranProperty='紧急(电话联系输血科)'` 的申请单数 |
| 常规 | `TranProperty='常规'` 的申请单数 |
| 备血 | `TranProperty='备血'` 的申请单数 |
| 其他状态异常（底层） | 日期范围内无法映射到已知状态的唯一申请单数，不进入正式总数 |
| 空申请单号异常（底层） | 查询范围内申请单号为空的物理行数 |
...
SELECT
    ...
    LTRIM(RTRIM(ISNULL(a.Remark, ''))) AS Remark,
...
汇总区只保留一个主汇总 ListView。主汇总上方增加自绘分组表头，用不同浅色区分同一申请单总数的两种分类维度：

主汇总：

| 总体 | 按申请状态分类 |  |  |  |  | 按紧急程度分类 |  |  |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 输血申请单总数 | 未审核 | 已审核 | 已完结 | 已驳回 | 已删除 | 紧急 | 常规 | 备血 |

主汇总的数据单元格与分组标题使用相同色系：总数浅灰、申请状态浅蓝、紧急程度浅橙；“紧急”数值额外使用红色文字强化提示。不显示两种分类合计关系提示、异常汇总 ListView 或异常状态文案，异常识别逻辑仅在底层保留。

“紧急、常规、备血”在申请单去重、院区过滤及“包含已驳回/包含已删除”过滤完成后，根据明细“紧急程度”使用的 `TranProperty` 精确统计：`紧急(电话联系输血科)` 计入紧急，`常规` 计入常规，`备血` 计入备血。
...
| --- | --- |
| 院区 | 根据申请科室派生 |
| 紧急程度 | `TranProperty`；值为 `紧急(电话联系输血科)` 时使用与输血结果查询一致的红色单元格 |
| 申请单号 | `ApplyFormNO` |
| 申请时间 | `Apply_Time` |
| 申请状态 | `ApplyForm_Statue` |
| 原因 | `Remark`，主要用于显示已驳回申请单的原因 |
| 病人号 | `Patient_NO` |
| 病人类型 | `Patient_NOType` |
| 姓名 | `Patient_Name` |
| 申请科室 | `Apply_Dept` |
...
“紧急程度”列单独复用“输血结果查询”左侧列表规则：去空格后的 `TranProperty` 精确等于 `紧急(电话联系输血科)` 时，仅该单元格覆盖为 `RGB(234,51,35)` 红色背景；其他单元格继续使用上述申请状态行底色。
...
```text
开始日期-结束日期输血单统计明细-{全部|老院|新院}.xlsx
```

导出为标准 OOXML Excel 工作簿，列顺序与页面一致，并包含“紧急程度”和“原因”列。文件名使用最后一次成功查询的日期和院区，避免查询失败后条件与数据不一致。
...
- 实现汇总、明细、状态配色、本地排序和 Excel `.xlsx` 导出；导出字段与明细列一致，包含“紧急程度”和“原因”。
- 实现双击申请单跳转。
...
| 紧急程度为 `紧急(电话联系输血科)` | 紧急增加 1，明细对应单元格显示红色背景 |
| 紧急程度为 `常规` | 常规增加 1 |
| 紧急程度为 `备血` | 备血增加 1 |
| 已驳回申请存在 `Remark` | 明细“原因”列显示原始驳回原因 |
